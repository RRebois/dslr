...
def train_logreg(df: DataFrame, titles: list) -> None:
    ...
    for house in tqdm(houses):
        ...
        for iter in range(n_iters):
            # Generate random index and get line
            rand_x = random.randrange(0, len(X) - 1, 1)
            x = X[rand_x]
            label = y[rand_x]

            y_pred = sigmoid(np.dot(x, theta) + intercept)

            dw = np.dot(x.T, (y_pred - label))
            db = y_pred - label

            theta -= lr * dw
            intercept -= lr * db
            loss = -(label * np.log(y_pred) + (1 - label) * np.log(1 - y_pred))#label * np.log(y_pred) + (1 - label) * np.log(1 - y_pred)
            cost.append(loss)
            if iter % 100 == 0:
    ...
    plt.ylabel('Cost')
    plt.title('Stochastic logistic regression cost')
    plt.savefig("Stochastic_cost.png")
    ...
    try:
        with open("stochastic_thetas.pkl", "wb") as f:
